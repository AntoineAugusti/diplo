...
public class CommunicationServeur {
	...
	try {
	    ...
	    System.out.println(" Le protocole utilisé est invalide :" +
	    e.getMessage());
	} catch (IOException e) {
    ...
    private String postRequete(String uri, String postData) throws
	...
	Jeu		jeuCourant = new Jeu(partie.getInt("id"), partie.getInt(
	    "nb_joueurs_requis"), partie.getInt("nb_joueurs_inscrits"));
	JSONObject	joueurJSON = obj.getJSONObject("joueur");
	Joueur		joueurCourant = new Joueur(joueurJSON.getInt("id"),
	    joueurJSON.getString("pseudo"), joueurJSON.getString("pays"), 0, 0);
	...
	for (int i = 0; i < listeJoueurs.length(); i++)
	    liste.add(new Joueur(listeJoueurs.getJSONObject(i).getInt("id"),
	    listeJoueurs.getJSONObject(i).getString("pseudo"),
	    listeJoueurs.getJSONObject(i).getString("pays"),
	    listeJoueurs.getJSONObject(i).getInt("armees_restantes"),
	    listeJoueurs.getJSONObject(i).getInt("cases_controlees")));
	return liste;
    }
	...
	case "NEGOCIATION":
	    phaseCourante = new Phase(Phase.Statut.NEGOCIATION, message.getInt(
		"chrono"));
	    break;
	case "COMBAT":
	    phaseCourante = new Phase(Phase.Statut.COMBAT, message.getInt(
		"chrono"));
	    break;
	default:
	    phaseCourante = new Phase(Phase.Statut.INACTIF, 1);
	}
	...
	for (int i = 0; i < listeCase.length(); i++) {
	    ...
	    JSONArray		voisines = listeCase.getJSONObject(
		i).getJSONArray("id_cases_voisines");
	    ...
	    carteCourante.ajouterCase(new Case(listeCase.getJSONObject(
	    i).getInt("id"), listeCase.getJSONObject(i).getBoolean(
	    "est_libre"), joueur_val, listeCase.getJSONObject(i).getBoolean(
	...
	for (int i = 0; i < messagesJSON.length(); i++)
	    messages.add(new Message(messagesJSON.getJSONObject(i).getInt("id"),
	    messagesJSON.getJSONObject(i).getInt("joueur"),
	    messagesJSON.getJSONObject(i).getString("texte"), new Date(
	    messagesJSON.getJSONObject(i).getString("created_at"))));
	return new Conversation(conv.getInt("id"), joueurs, messages);
    }

    private ArrayList<Conversation> parserJSONInfosConversationSelonJoueur(
    String fichier) {
	JSONObject	conv = new JSONObject(fichier);
	...
	for (int i = 0; i < liste.length(); i++) {
	    ...
	    JSONArray		joueursJSON = liste.getJSONObject(
		i).getJSONArray("joueurs");

	    for (int j = 0; j < joueursJSON.length(); j++)
		joueurs.add(joueursJSON.getInt(j));
	    result.add(new Conversation(liste.getJSONObject(i).getInt("id"),
	    joueurs, null));
	}
	return result;
    }
	...
	return new Message(sms.getInt("id"), sms.getInt("id_joueur"),
	    sms.getString("texte"), new Date(sms.getString("created_at")));
    }
	...
	for (int i = 0; i < armees.length(); i++)
	    resultat.add(new Armee(armees.getJSONObject(i).getInt("id"),
	    armees.getJSONObject(i).getInt("id_joueur"),
	    armees.getJSONObject(i).getInt("id_case")));
	return resultat;
    }
    ...
    PartieIntrouvableException, PartieInvalideException, RuntimeException {
	Phase	phaseCourante = null;
	String	reponse = "";
    ...
    public Conversation recupererInfosConversation(int conversationID) throws
    PartieIntrouvableException, RuntimeException {
	...
	try {
	    ...
	    reponse = postRequete("conversations", conversationToJSON(
		destinataires).toString());
	...
	try {
	    reponse = postRequete("conversations/" + conversationID +
		"/messages", smsToJSON(auteur, texte).toString());
