...
public class CommunicationServeur {
	...
	private String getRequete(String uri) throws PartieHTTPSException {
		...
		try {
			...
			System.out.println(
			" Le protocole utilisé est invalide :" +
			e.getMessage());
		} catch (IOException e) {
	...
	private Phase parserJSONInfosPhase(String fichier) {
		...
		switch (message.getString("phase")) {
			case "NEGOCIATION":
				phaseCourante = new Phase(
				Phase.Statut.NEGOCIATION, message.getInt(
				"chrono"));
				break;
			case "COMBAT":
				phaseCourante = new Phase(Phase.Statut.COMBAT,
				message.getInt("chrono"));
				break;
			default:
				phaseCourante = new Phase(Phase.Statut.INACTIF,
				1);
		}
	...
	String fichier) {
		...
		for (int i = 0; i < liste.length(); i++) {
			...
			result.add(new Conversation(liste.getJSONObject(
			i).getInt("id"), joueurs, null));
		}
		return result;
	}

	private JSONObject conversationToJSON(
	...
	private ArrayList<Armee> parserJSONInfosArmee(String fichier) {
		...
		for (int i = 0; i < armees.length(); i++)
			resultat.add(new Armee(armees.getJSONObject(i).getInt(
			"id"), armees.getJSONObject(i).getInt("id_joueur"),
	...
	PartieIntrouvableException, PartiePleineException {
		...
		try {
			reponse = postRequete("parties/" + partieID +
				"/rejoindre", "");
	...
	PartieIntrouvableException, RuntimeException {
		...
		try {
			reponse = getRequete("parties/" + partieID +
				"/joueurs");
	...
	joueurID) throws PartieIntrouvableException {
		...
		try {
			reponse = getRequete("conversations/joueurs/" +
				joueurID);
	...
	texte) throws PartieInvalideException {
		...
		try {
			reponse = postRequete("conversations/" +
				conversationID + "/messages", smsToJSON(auteur,
				texte).toString());
