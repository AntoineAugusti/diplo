...
public class JSONTests {
	...
	for (int i = 0; i < listeCase.length(); i++) {
	    ...
	    carte.ajouterCase(new Case(listeCase.getJSONObject(i).getInt("id"),
	    listeCase.getJSONObject(i).getBoolean("est_libre"), joueur_val,
	    listeCase.getJSONObject(i).getBoolean("est_occupee"), armee_val,
	    null));
	}
	return carte;
    }
	...
	assert infosJoueurs.get(0).id == 1 : "L'id du premier joueur vaut " +
	infosJoueurs.get(0).id;
	assert infosJoueurs.get(0).pseudo.equals(
	"Fake"): "Le pseudo du premier joueur vaut " + infosJoueurs.get(
	    0).pseudo;
	assert infosJoueurs.get(0).pays.equals(
	"FRA"): "Le pays du premier joueur vaut " + infosJoueurs.get(
	    0).pays;
	...
	assert infosJoueurs.get(1).id == 2 : "L'id du second joueur vaut " +
	infosJoueurs.get(1).id;
	assert infosJoueurs.get(1).pseudo.equals(
	"Dummy"): "Le pseudo du second joueur vaut " + infosJoueurs.get(
	    1).pseudo;
	assert infosJoueurs.get(1).pays.equals(
	"GBR"): "Le pays du second joueur vaut GBR" + infosJoueurs.get(
	    1).pays;
	...
	assert partieRejointe.getRequis() == 5 : "Le jeu a un requis de " +
	partieRejointe.getRequis();
	assert partieRejointe.getInscrits() == 3 : "Le jeu a un inscrits de 3" +
	...
	assert joueurs.get("Blah").id == 2 : "Le joueur Blah a un id de " +
	joueurs.get("Blah").id;
	assert joueurs.get("Blah").pays.equals(
	"FRA"): "Le joueur Blah a FRA en pays " + joueurs.get("Blah").pays;
	assert joueurs.get("Blah").pseudo.equals(
	"Blah"): "Le joueur Blah a en pseudo " + joueurs.get("Blah").pseudo;
    ...
    false: "La case 1 a pour est_libre " + carte.getCase(1).est_libre;
	assert carte.getCase(1).id_joueur == 1 : "La case 1 a pour id_joueur " +
	carte.getCase(1).id_joueur;
	assert carte.getCase(1).est_occupee ==
    false: "La case 1 a pour est_occupee " + carte.getCase(
	1).est_occupee;
	assert carte.getCase(1).id_armee == 0 : "La case 1 a pour id_armee " +
	carte.getCase(1).id_armee;
	assert carte.getCase(2).id == 2 : "La case 2 a pour id " +
	carte.getCase(2).id;
	assert carte.getCase(2).est_libre ==
    true: "La case 2 a pour est_libre " + carte.getCase(2).est_libre;
	assert carte.getCase(2).id_joueur == 0 : "La case 2 a pour id_joueur " +
	carte.getCase(2).id_joueur;
	assert carte.getCase(2).est_occupee ==
    true: "La case 2 a pour est_occupee " + carte.getCase(
	2).est_occupee;
	assert carte.getCase(2).id_armee == 1 : "La case 2 a pour id_armee " +
	carte.getCase(2).id_armee;
	...
	"Il n'y a pas assez de joueurs pour démarrer la partie"):
	    "Le message associé au statut est : " + statut.getMessage();
    }
}
